...
def test_fetch_and_write():
    ...
    # 3. Normalize (snake_case internally)
    print("Normalizing positions...")
    # Normalize expects snake_case input from fetch_positions
    # fetch_positions already returns snake_case internally
    today_iso = date.today().isoformat()
    positions_df = pipeline.normalize_positions(raw_positions, import_date=today_iso, source="schwab_api")
    
    # 4. Write to Sheets (DRY_RUN is False now in config.py)
    ...
    if results['holdings_written'] > 0:
        print("\n✅ SUCCESS: Schwab API data has been sent to your Google Sheet.")
        return True
    else:
