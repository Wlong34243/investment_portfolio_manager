...
def analyze_cash_position(holdings_df: pd.DataFrame) -> dict:
    ...
    # In this app, config.DEFAULT_CASH_YIELD_PCT is used for manual cash
...
def generate_cash_deployment_suggestion(cash_analysis: dict, holdings_df: pd.DataFrame) -> dict:
    ...
    system_instruction = f"{SAFETY_PREAMBLE}\n\nYou are a yield optimization advisor. Suggest reallocation of idle cash to higher-yielding alternatives. Respond ONLY with JSON: {{'recommendation': str, 'proposed_action': str, 'yield_improvement': str, 'risk_note': str}}"
...
def get_cash_sweep_alert(holdings_df: pd.DataFrame) -> str:
    ...
    if total_val > 0 and (cash_val / total_val * 100) > 5.0:
        # Check if we have high-yield alternatives in holdings or just a general alert
