...
def main_dashboard():
        ...
        if not df.empty:
            ...
            if 'Import Date' in df.columns:
                try:
                    ...
                    if days_old < 1:
                        st.success(f"🟢 Fresh — imported today")
                    elif days_old <= 7:
                        st.warning(f"🟡 {days_old} days old")
                    else:
                        st.error(f"🔴 {days_old} days old — re-import")
                except:
                    st.caption("Freshness: Unknown")
    ...
    with tabs[1]:
        if st.session_state["holdings_df"].empty:
            ...
            fig_income = px.bar(top_gen, x='Est Annual Income', y='Ticker', orientation='h', title='Top 10 Income Generators')
            ...
            if movers:
                with st.expander(f"🚀 Daily Movers ({len(movers)} active)", expanded=True):
                    if st.button("🎙️ Explain Movements with AI"):
                        with st.spinner("AI is checking catalysts..."):
