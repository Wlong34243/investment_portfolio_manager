...
- ~~**`PortfolioUI` logon task registers fine but the process dies silently under `pythonw.exe`**~~ **FIXED 2026-08-28.** Root cause confirmed: under `pythonw`, `sys.stdout`/`sys.stderr` are `None`, not just discarded, so the first `print()`/`console.print()`/logging call anywhere throws `AttributeError` and kills the process with no trace. `manager.py` now detects `sys.stdout is None or sys.stderr is None` at the very top (before any other import) and redirects both to `logs/pythonw_stdio.log` (line-buffered, UTF-8, `errors="replace"`) — ahead of the existing cp1252-reconfigure loop, which now has real streams to work with. Verified both ways: direct `pythonw.exe manager.py ui serve` (PID stayed alive past 5s, `curl` got 200, log file populated with the full startup banner + uvicorn INFO lines) and through the actual `Start-ScheduledTask -TaskName PortfolioUI` path (`/runs` returned 200). The fix is global (top of `manager.py`, not scoped to `ui serve`), so it covers any future pythonw-launched command, not just this one.
